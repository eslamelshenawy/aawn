...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">{{trans('admin.allagent')}}</h5>
        ...
        <table class="table datatable-button-html5-columns">
            ...
            @foreach($agents as $agent)
                <tr>
                    <td>{{$agent->id}}</td>
                    <td>{{$agent->name}}</td>
                    <td><a href="{{aurl('countries/city_agents/'.$agent->country_id)}}">{{$agent->agent_country()->first()->country_name_ar ?? "" }}</a></td>
                    <td>{!! $agent->location!!}</td>
                    <td>{{$agent->phone}}</td>
                    <td><a href="{{'agents/edit/'.$agent->id}}"><i class="icon-pen6"></i> <span>edit</span></a></td>
